...
- `POST /webhooks/meta/instagram`: recibe eventos de Instagram, responde `200 OK` inmediatamente y procesa en segundo plano. Si el evento incluye `message.text`, crea o reutiliza contacto por Instagram scoped ID, crea o reutiliza conversacion Instagram abierta, guarda mensaje inbound, actualiza `lastMessageAt` y registra auditoria. Si el evento solo trae informacion parcial, por ejemplo `message_edit`, usa el Page Access Token de `META_INSTAGRAM_ACCESS_TOKEN` para consultar `me/conversations?platform=instagram` y sincronizar mensajes nuevos desde `messages{message,from,created_time}`. Usa `META_INSTAGRAM_ORGANIZATION_ID` para resolver la organizacion destino.

## Futuro

- Mensajes WhatsApp con plantillas y adjuntos.
